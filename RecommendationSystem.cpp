...
std::vector<double> RecommendationSystem::get_preference_vector(const User& user) const {
    ...
    for (const auto& r : ranks) {
        ...
        double s_bar = r.second - m;
        for (size_t i = 0; i < dim; i++) {
            p[i] += s_bar * it->second[i];
        }
    }
    return p;
}
...
sp_movie RecommendationSystem::recommend_by_content(const User& user) const {
    ...
    for (const auto& kv : movies_features) {
        ...
        if (sim > best_sim ||
            (sim == best_sim && best_movie && *kv.first < *best_movie)) {
            best_sim = sim;
            best_movie = kv.first;
        }
    }
...
double RecommendationSystem::predict_movie_score(const User& user,
    ...
    if (movie_it == movies_features.end()) return 0;
    ...
    for (int i = 0; i < k; i++) {
        num += neighbors[i].first * (neighbors[i].second - m);
        den += std::abs(neighbors[i].first);
    }
...
sp_movie RecommendationSystem::recommend_by_cf(const User& user, int k) {
    ...
    for (const auto& mv : movies_features) {
        ...
        if (score > best_score ||
            (score == best_score && best_movie && *mv.first < *best_movie)) {
            best_score = score;
            best_movie = mv.first;
        }
    }
